...
$get_tasks = Get-ScheduledTask | Where State -ne "Disabled" | Select TaskName,Description,TaskPath
...
foreach ($task in $get_tasks) {
	...
	# output data
	write "${wildvalue}##${task_name}##${task_description}####auto.TaskPath=${task_path}"
}
